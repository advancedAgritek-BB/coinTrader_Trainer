...
def test_upload_uses_joblib_and_validates_metrics(monkeypatch, registry_with_dummy):
    ...
    monkeypatch.setattr(registry, 'joblib', types.SimpleNamespace(dump=fake_dump, load=lambda f: None), raising=False)
...
def test_get_latest_uses_joblib_load(monkeypatch, registry_with_dummy):
    ...
    monkeypatch.setattr(registry, 'joblib', types.SimpleNamespace(dump=lambda o,f: None, load=fake_load), raising=False)
